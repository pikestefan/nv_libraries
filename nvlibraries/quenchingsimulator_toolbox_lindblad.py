...
Sx = np.array([[0, 1, 0], [1, 0, 1], [0, 1, 0]], dtype=complex) / scalar_sqrt(2)
...
def zerofield_kraus(
    k30,
    k41,
    k52,  # Radiative decay rates
    k36,
    k46,
    k56,  # Nonrad. decays to the intersystem crossing
    k60,
    k61,
    k62,  # Nonrad. decays from the intersystem crossing
    laser_pump=0,
    kmw_01=0,
    kmw_02=0,
):
...
def lindblad_quench_simulator(
    Bfields=None, rate_dictionary=None, nv_theta=54.7, nv_phi=0, Bias_field=None
):
    """
    ...
    default_rate_dictionary = {
        "kr": 65,  # The radiative decay rate
        ...
        "laser_pump": 0.1,
    }  # Laser driving, percentage of kr
    ...
    zerofield_krausop = zerofield_kraus(
        kr,
        kr,
        kr,
        k36,
    ...
    Bfields = calculate_projection_nvaxis(
        fields2project=Bfields, nv_theta=nv_theta, nv_phi=nv_phi
    )
    ...
    sum_Lk_rho_Lkdag = np.einsum(
        "ijk,imn->kmjn", zerofield_krausop_adj, zerofield_krausop
    )

    decoherence_lindblad = sum_Lk_rho_Lkdag - 0.5 * (
        sum_Lkdag_Lk_rho + sum_rho_Lkdag_Lk
    )

    ham_commutator = np.einsum(
        "ij,mlk->miljk", identity, master_hamiltonian
    ) - np.einsum("ijk,mn->ijnkm", master_hamiltonian, identity)

    lindbladian = (ham_commutator * (-1j * 2 * np.pi) + decoherence_lindblad).reshape(
        bfield_num, kraus_rows ** 2, kraus_cols ** 2
    )
    ...
    solution_column = np.repeat(
        solution_column[None, :, None], repeats=bfield_num, axis=0
    )

    final_solution = (
        np.linalg.solve(lindbladian, solution_column)
...
def calculate_projection_nvaxis(fields2project=None, nv_theta=0, nv_phi=0):
    ...
    nv_unit_vec = np.array(
        [cos(nv_phi) * sin(nv_theta), sin(nv_phi) * sin(nv_theta), cos(nv_theta)]
    )
...
if __name__ == "__main__":
    ...
    ang = (
        0.1 * np.pi / 180
    )  # np.linspace(0,10*np.pi/180,1000)#np.linspace(0,np.pi/2,1000)
    ang2 = 0 * np.pi / 180
    ...
    Bfields = np.stack(
        (
            norms * cos(ang2) * np.sin(ang),
            norms * sin(ang2) * np.sin(ang),
            np.cos(ang) * norms,
        ),
        axis=1,
    )
    ...
    def rate_dictionary_func(coeff):
        r8 = {
            "kr": 32.2 * coeff,  # The radiative decay rate
    ...
    for ii, laspump in enumerate(powers):
        pl, poppy, hey = lindblad_quench_simulator(
            Bfields[0, :], nv_theta=0, rate_dictionary={"laser_pump": laspump}
        )
        poppytot[ii, :] = poppy
        pltot[ii] = pl
