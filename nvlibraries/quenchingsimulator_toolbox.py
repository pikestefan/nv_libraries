...
def quenching_simulator(
    Bfields,
    rate_dictionary=None,
    nv_theta=54.7 * np.pi / 180,
    nv_phi=0,
    rate_coeff=1e-3,
    Bias_field=None,
    ...
    # field components). Allows to handle any matrix dimensionality.
        ...
        if input_shape[-1] != 3:
            raise ValueError(
                "The last matrix dimension must contain the 3 "
    ...
    Bfields = rotate2nvframe(
        vectors2transform=Bfields, nv_theta=nv_theta, nv_phi=nv_phi
    )
    ...
    # (bfield_num, 7, 7)
    coefficient_matrix = find_eigens_and_compose(
        Htot_gs=full_Htot_gs,
    ...
    new_rates = np.matmul(
        np.square(np.abs(coefficient_matrix)),
        np.matmul(
            zero_rates,
    ...
    solution_vector = np.repeat(
        solution_vector[np.newaxis, :, np.newaxis], bfield_num, axis=0
    )
...
def generate_rate_eq(decay_matrix):
    ...
    rate_matrix[:, di_inds[0], di_inds[1]] = rate_matrix[
        :, di_inds[0], di_inds[1]
...
def find_eigens_and_compose(Htot_gs=None, Htot_es=None, correct_for_crossing=False):
    ...
    if correct_for_crossing:
        ...
        GS_ground_state_coeff_pos = np.argmax(
            np.square(np.abs(eistates_gs[:, 1, :])), axis=1
        )
        sub = eistates_gs[GS_ground_state_coeff_pos > 0, :, :]
        sub[:, :, [0, 1]] = sub[:, :, [1, 0]]
        eistates_gs[GS_ground_state_coeff_pos > 0, :, :] = sub

        ES_ground_state_coeff_pos = np.argmax(
            np.square(np.abs(eistates_es[:, 1, :])), axis=1
        )
...
def rotate2nvframe(vectors2transform, nv_theta=0, nv_phi=0):
    ...
    if (not isinstance(nv_theta, (np.ndarray, list))) and (
        not isinstance(nv_phi, (np.ndarray, list))
    ):
...
if __name__ == "__main__":
    ...
    pl, pops = quenching_simulator(
        Bfields, Bias_field=[0, 0, 1e-3], nv_theta=[0, 0.1], correct_for_crossing=True
    )
